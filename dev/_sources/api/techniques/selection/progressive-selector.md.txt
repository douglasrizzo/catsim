...
At test position $n$ out of a fixed total length $N$, the candidate
score for item $i$ is
...
where $R_i$ is a fresh uniform random draw on $[0,1]$ and
...
with acceleration parameter $s > 0$. The selector chooses the
