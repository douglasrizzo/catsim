...
Let $N$ be the configured test size and $J$ the number of items in
the bank. The bank is divided into $N$ contiguous strata by equally spaced
slice boundaries
...
At test position $k+1$, the selector operates only on the stratum
...
```{eval-rst}
.. autoclass:: catsim.selection.StratifiedSelector
   :members:
   :show-inheritance:
```
