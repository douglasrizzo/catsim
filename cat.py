...
def simCAT(items, clusters, n_itens=20, tests=1):
    ...
    item selection rules in computerized adaptive testing. Applied Psychological
    ...
    #10 maximum exposure rates extracted from a linear interval rangin from .1 to 1
    ...
    for r_max in r_maxes:
        for true_theta in true_thetas:
            ...
            # keeps indexes of items that were already administered for this examinee
            administered_items = set()

            response_vector = []

            for q in np.arange(n_itens):
                # iterates through all items, looking for the item that has the biggest information value, given the estimated theta
                selected_item = None
                max_inf = 0
                for counter, i in enumerate(items):
                    if counter not in administered_items and irt.inf(
                        ...
                        # procura o que tem maior informação e que ainda não
                        # foi administrado
                        i[2]) > max_inf:
                ...
                if items[counter, 3] >= r_max:
                    ...
                    random_item = None
                    while random_item == None:
                        random_item = random.randint(0, np.size(items, 0))
                        if selected_item_cluster == clusters[random_item] and random_item not in administered_items:
                            selected_item = random_item
                        else:
                            random_item = None

                # simulates the examinee's response via the three-parameter logistic function
...
def dodd(theta, items, acertou):
    """
    Method proposed by [Dodd1990] for the reestimation of :math:`\\hat{\\theta}`
    ...
    .. [Dood1990] Dodd, B. G. (1990). The Effect of Item Selection Procedure and Stepsize on Computerized Adaptive Attitude Measurement Using the Rating Scale Model. Applied Psychological Measurement, 14(4), 355–366. http://doi.org/10.1177/014662169001400403
    """
    b = items[:, 1]
    b_max = max(b)
    b_min = min(b)
