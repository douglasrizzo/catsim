...
class Simulator:
    ...
    def simulate(
        self, items: np.ndarray, examinees, initialization: Initializer, selection: Selector,
        reestimation: Estimator, stopping: Stopper
    ):
        ...
        :param examinees: an integer with the number of examinees, whose real :math:`\\theta` values will be
                          ...
                          :math:`\\theta_0` values
        :param initialization: an initializer that selects examinees :math:`\\theta_0`
        :param selection: a selector that selects new items to be presented to examinees
        :param reestimation: an estimator that reestimates examinees proficiencies after each item is applied
        :param stopping: an object with a stopping criteria for the test
        """

        items = irt.validate_item_bank(items)
        ...
        if type(examinees) == int:
            ...
            true_thetas = np.array(examinees)

        if true_thetas.shape[1] > 1:
            raise ValueError('examinees cannot be a multimensional data structure')

        for true_theta in true_thetas:
            est_theta = initialization.initialize()
            response_vector, administered_items = [], []

            while not stopping.stop(len(administered_items)):
                selected_item = selection.select(items, administered_items, est_theta)

                # simulates the examinee's response via the three-parameter
                # logistic function
                response = irt.tpm(
                    true_theta, items[selected_item][0], items[selected_item][1],
                    items[selected_item][2]
                ) >= np.random.uniform()
                ...
                if len(set(response_vector)) == 1:
                    ...
                    est_theta = reestimation.reestimate(
                        response_vector, administered_items, est_theta
                    )

                # update the exposure value for this item
                items[administered_items, 3] = (
                    (items[administered_items, 3] * examinees) + 1
                ) / examinees
            ...
            #
            #     if r_max > 1:
            #         raise ValueError('r_max must be greater than 0 and lesser or equal to 1')
            #     if items.shape[1] != 3:
            ...
            #         raise ValueError('Invalid method, select one from' + str(self.existent_methods) + '.')
            #     if method in self.cluster_dependent_methods and clusters is None:
            #         raise ValueError('Method {0} cannot be used when clusters is None'.format(method))
            ...
            #     items = np.append(
            #         items,
            #         np.array(clusters).reshape(clusters.shape[0], 1),
            #         axis=1
            #     ).astype(np.float64)
            #
            #     localResults = []
            ...
            #
            #                     inf_values = [
            #                         irt.inf(est_theta, i[0], i[1], i[2]) for i in items[valid_indexes]
            #                     ]
            #
            #                     valid_indexes = [
            #                         index
            ...
            #                             cluster_infos = sum_cluster_infos(est_theta, items, clusters)
            #                         elif method == 'weighted_info':
            #                             cluster_infos = weighted_cluster_infos(est_theta, items, clusters)
            #
            ...
            #                                     zip(cluster_infos, set(clusters)),
            #                                     reverse=True
            #                                 )
            #                             ],
            #                             dtype=float
            ...
            #                     # administered
            #                     valid_indexes_low_r = np.array(
            #                         list(
            #                             set(
            #                                 np.nonzero(
            #                                     (items[:, 4] == selected_cluster) & (items[:, 3] < r_max)
            #                                 )[0]
            #                             ) - set(administered_items)
            #                         )
            #                     )
...
if __name__ == '__main__':
    ...
    Simulator().simulate(generate_item_bank(100), 1000, initializer, selector, estimator, stopper)
    # import doctest
    # doctest.testmod()
