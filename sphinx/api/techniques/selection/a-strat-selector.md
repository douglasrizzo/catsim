...
unlock higher-discrimination items once $\hat{\theta}$ is more reliable.
...
Let $a_i$ be the discrimination parameter of item $i$. The package
...
the generic stratified slicing rule. At test position $k+1$, the selector
returns the first non-administered item from stratum $k$.

This is a simplified operational form of the a-stratified idea: strata are based
on ascending $a$, and selection inside the active stratum follows the fixed
presorted order.
