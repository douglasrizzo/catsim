...
Let $m$ be the number of administered items. The package checks stopping in
the following order:
...
where $J$ is the size of the item bank. If a minimum length
$m_{\min}$ is configured and $m < m_{\min}$, the rule returns
