...
Let $\hat{\theta}_n$ be the current MLE after $n$ items, with cut
score $\theta_c$ and indifference half-width $\delta$. Define
...
where $\ell_n(\theta) = \log L(\mathbf{u} \mid \theta)$. A practical first
threshold is
