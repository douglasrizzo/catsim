...
For a response vector $\mathbf{u}$ on administered items with parameters
$\boldsymbol{\xi}$, the posterior is
...
On a fixed grid $\{\theta_q\}_{q=1}^{Q}$ with quadrature weights
$\{w_q\}_{q=1}^{Q}$, the discrete approximation is
