...
Let :math:`\hat{\theta}` be the current estimate, :math:`\mathcal{A}` the set
of administered items, and :math:`\mathcal{R}` the remaining items. Let
...
target :math:`\tau`, or if the predicted SE after adding the best remaining item
is still greater than :math:`\tau`.
