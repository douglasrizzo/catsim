...
:math:`I_i^{\max}`. After partitioning that order into stage strata, it postsorts
items *within the active stratum* by current information
:math:`I_i(\hat{\theta})` in descending order, and returns the first
non-administered item.

Equivalently, at position :math:`k+1`, selection is
...
where :math:`S_k` is the active MIS stratum induced by ordering items on
:math:`I_i^{\max}`.
...
.. seealso::
   ...
   API reference: :py:class:`catsim.selection.MaxInfoStratSelector`
