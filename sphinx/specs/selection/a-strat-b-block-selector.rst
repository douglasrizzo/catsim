...
are correlated, because strata formed only by :math:`a` may also inherit skewed
...
Let :math:`b_i` be item difficulty and :math:`a_i` item discrimination. The
package implementation performs the following presort:

1. Sort all items by increasing :math:`b_i`.
2. Partition that ordering into ``test_size`` contiguous strata.
3. Within each stratum, sort items by increasing :math:`a_i`.
4. Concatenate the reordered strata and use the generic stratified family rule.

At position :math:`k+1`, the selector returns the first non-administered item in
the :math:`k` th stratum of this final ordering.
...
.. seealso::
   ...
   API reference: :py:class:`catsim.selection.AStratBBlockSelector`
